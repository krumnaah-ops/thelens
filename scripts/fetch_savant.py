...
def fetch_pitch_tempo():
    """
    ALL pitchers SB stats + runners-on splits in TWO bulk API calls.
    Call 1: /stats?stats=season&group=pitching  → SB allowed, CS, IP
    Call 2: /stats?stats=statSplits&sitCodes=ro → runners-on ERA/OBP/WHIP
    """
    print('\n[8/11] Pitcher SB metrics — BULK (two API calls)...')

    # Call 1: season pitching stats (SB allowed, CS, IP)
    season_data = mlb_get(
        f'stats?stats=season&group=pitching&season={SEASON}&sportId=1&limit=500'
    )

    # Call 2: runners-on splits for all pitchers
    ro_data = mlb_get(
        f'stats?stats=statSplits&group=pitching&season={SEASON}&sportId=1&sitCodes=ro&limit=500'
    )

    # Build season stats map
    season_map = {}
    if season_data:
        for split in season_data.get('stats', [{}])[0].get('splits', []):
            pid = split.get('player', {}).get('id')
            if pid:
                season_map[pid] = split.get('stat', {})

    # Build runners-on map
    ro_map = {}
    if ro_data:
        for split in ro_data.get('stats', [{}])[0].get('splits', []):
            pid = split.get('player', {}).get('id')
            if pid:
                ro_map[pid] = split.get('stat', {})

    result = {}
    for pid, ss in season_map.items():
        ...
        # Convert MLB IP format (5.1 = 5⅓) to decimal
        ip_whole = int(ip_raw)
        ip_frac  = (ip_raw - ip_whole) * 10
        ip_dec   = ip_whole + (ip_frac / 3)

        sb_per9 = round((sb_allowed / ip_dec) * 9, 2) if ip_dec > 0 and sb_allowed is not None else None

        # Derive pace proxy from SB/9
...
def main():
    ...
    if errors:
        ...
        core = [e for e in errors if e in ('arsenal', 'pitcher-expected', 'hitter-expected', 'hitter-statcast')]
        if core:
            sys.exit(1)
